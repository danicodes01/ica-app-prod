...
export default function GameCanvas() {
  ...
  // Handle canvas size and initial setup
  useEffect(() => {
  ...
  // Load planets when dimensions change
  useEffect(() => {
    if (dimensions.width === 0 || dimensions.height === 0 || !session?.user) return;
    loadPlanets(session.user.totalXP || 0, dimensions.width, dimensions.height);
  }, [dimensions, session?.user, loadPlanets]);

  // Handle renderer setup and animation
  useEffect(() => {
    if (!canvasRef.current) return;
    ...
    rendererRef.current.startAnimation({
      ...
      canvas,
      colors: GAME_COLORS,
      state: { planets, currentPlanet: null, playerPosition, isIntroComplete }
    });

    return () => {
      rendererRef.current?.stopAnimation();
    };
  }, [planets, playerPosition, canvasOpacity, isIntroComplete]);
  ...
  const handleMouseMove = useCallback((event: MouseEvent) => {
    ...
    const newX = Math.max(20, Math.min(dimensions.width - 20, event.clientX - rect.left));
    const newY = Math.max(20, Math.min(dimensions.height - 20, event.clientY - rect.top));
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (['a', 'w', 's', 'd', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
      ...
      if (e.code === 'Space' && hoveredPlanet) {
        ...
        if (distanceToPlayer <= hoveredPlanet.position.radius + INTERACTION_RADIUS && hoveredPlanet.isUnlocked) {
          handlePlanetInteraction(hoveredPlanet);
          router.push(`/game/planets/${hoveredPlanet.id}`);
        }
      }
  ...
  // Game loop for keyboard movement
  useEffect(() => {
  ...
  }, [dimensions, keys, isMouseMoving, playerPosition, checkPlanetProximity, setPlayerPosition]);
