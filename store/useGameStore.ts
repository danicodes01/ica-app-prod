...
import { createPlanetSlice } from './slices/planetSlice';
import { createCanvasSlice } from './slices/canvasSlice';
import { createProgressSlice } from './slices/progressSlice';
import { GameStore, UserStats } from '@/types/store/slices';
...
export const useGameStore = create<GameStore>((set, get, store) => ({
  ...initialState,
  ...createPlanetSlice(set, get, store),
  ...createCanvasSlice(set, get, store),
  ...createProgressSlice(set, get, store),

  initializeGameState: async (stats: UserStats) => {
    try {
      // Set initial user stats from the session
      set({ userStats: stats });
      
      // Load planets and user data from database
      await get().loadDBPlanets();

      if (typeof window !== 'undefined' && get().dbPlanets.length > 0) {
...
//       initializeGameState: async () => {  // Remove stats parameter
//         try {
//           if (!get().isHydrated) {
